-- Fix public association search RPC ambiguous column references.
...
begin
  ...
  if normalized_query <> '' and not resolved_location then
    select avg(a.latitude), avg(a.longitude), min(a.city), count(distinct a.province)
    ...
    where a.status = 'active'
      ...
      and public.normalize_lookup_text(a.city) = normalized_query;
    ...
    if not resolved_location then
      select a.latitude, a.longitude, coalesce(a.city, search_query_value), 1
      into origin_latitude, origin_longitude, resolved_origin_label, city_province_count
      from public.associations a
      where a.status = 'active'
        and a.geocode_status = 'geocoded'
        ...
        and public.normalize_lookup_key(a.postal_code) like normalized_key || '%'
    ...
    if not resolved_location then
      select avg(a.latitude), avg(a.longitude), min(a.city), 1
      into origin_latitude, origin_longitude, resolved_origin_label, city_province_count
      from public.associations a
      where a.status = 'active'
        and a.geocode_status = 'geocoded'
        and a.latitude is not null
        and a.longitude is not null
        and public.normalize_lookup_text(coalesce(a.street_address, '') || ' ' || coalesce(a.city, '') || ' ' || coalesce(a.postal_code, '')) like '%' || normalized_query || '%';
