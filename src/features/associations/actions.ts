...
export async function updateAdminAssociationRecord(_previousState: AssociationActionState = INITIAL_ERROR_STATE, formData: FormData): Promise<AssociationActionState> {
  ...
  const parsed = adminAssociationRecordUpdateSchema.safeParse({
    ...
    commonNameEn: valueFromFormData(formData, 'commonNameEn'),
    commonNameFr: valueFromFormData(formData, 'commonNameFr'),
    contactEmail: valueFromFormData(formData, 'contactEmail'),
    description: valueFromFormData(formData, 'description'),
    descriptionEn: valueFromFormData(formData, 'descriptionEn'),
    descriptionFr: valueFromFormData(formData, 'descriptionFr'),
  ...
  if (!parsed.success) {
    ...
    return {
      ...
      fieldErrors: {
        ...
        commonNameEn: flattened.commonNameEn === undefined ? undefined : 'KMG-RG-001',
        commonNameFr: flattened.commonNameFr === undefined ? undefined : 'KMG-RG-001',
  ...
  const { error: updateError } = await adminSupabase
    ...
    .update({
      ...
      common_name_en: optionalValue(parsed.data.commonNameEn ?? ''),
      common_name_fr: optionalValue(parsed.data.commonNameFr ?? ''),
      ...
      description_en: optionalValue(parsed.data.descriptionEn ?? ''),
      description_fr: optionalValue(parsed.data.descriptionFr ?? ''),
      ...geocoding,
      name: displayName,
